...
void build(int from, int to, int cf) {
    e[etp].to = to;
    e[etp].cf = cf;
...
bool bfs() {
    ...
    while (!bfs_q.empty()) {
        int now = bfs_q.front(); bfs_q.pop();
        for (int i = head[now]; i != -1; i = e[i].next) {
            int p = e[i].to;
            // 因为反向BFS, 所以需要判断反边是否存在于原网络
            if (e[i^1].cf != 0 && height[p] /*> height[now] + 1*/ == 0x3f3f3f3f) {
...
void push_flow(int now) {
    for (int i = head[now]; i != -1; i = e[i].next) {
        ...
        if (e[i].cf != 0 && height[now] == height[p] + 1) {
            int detla = min(excess_flow[now], e[i].cf);
            e[i].cf -= detla;
            e[i^1].cf += detla;
